---
name: launch
description: "Launch and automate VS Code (OmniCode) using @playwright/cli via Chrome DevTools Protocol. Use when you need to interact with the VS Code UI, automate the chat panel, test UI features, or take screenshots of VS Code. Triggers include 'automate VS Code', 'interact with chat', 'test the UI', 'take a screenshot', 'launch OmniCode with debugging'."
metadata:
...
Automate VS Code (OmniCode) using `@playwright/cli`. VS Code is built on Electron/Chromium and exposes a Chrome DevTools Protocol (CDP) port that `@playwright/cli` can attach to, enabling the same snapshot-interact workflow used for web pages.
...
- **For OmniCode (VS Code dev build):** The repo must be built before launching. `./scripts/code.sh` runs the build automatically if needed, or set `VSCODE_SKIP_PRELAUNCH=1` to skip the compile step if you've already built.
...
1. **Launch** OmniCode with remote debugging enabled
...
> SCREENSHOT_DIR="/tmp/omnicode-screenshots/$(date +%Y-%m-%dT%H-%M-%S)"
...
## Launching OmniCode (VS Code Dev Build)

The VS Code repository includes `scripts/code.sh` which launches OmniCode from source. It passes all arguments through to the Electron binary, so `--remote-debugging-port` works directly:
...
- OmniCode uses the default user data directory. Unlike VS Code Insiders, you don't typically need `--user-data-dir` since there's usually only one OmniCode instance running.
- If you see "Sent env to running instance. Terminating..." it means OmniCode is already running and forwarded your args to the existing instance. Quit OmniCode and relaunch with the flag, or use `--user-data-dir=/tmp/omnicode-debug` to force a new instance.

## Launching the Agents App (Agents Window)

The Agents app is a separate workbench mode launched with the `--agents` flag. It uses a dedicated user data directory to avoid conflicts with the main OmniCode instance.
...
**After making changes to OmniCode source code, you must restart to pick up the new build.** The workbench loads the compiled JavaScript at startup — changes are not hot-reloaded.

### Restart Workflow

1. **Rebuild** the changed code
2. **Kill** the running OmniCode instance
...
# 2. Kill the OmniCode instance listening on the debug port (if running)
pids=$(lsof -t -i :9224)
if [ -n "$pids" ]; then
	kill $pids
fi
...
> **Tip:** If you're iterating frequently, run `npm run watch` in a separate terminal so compilation happens automatically. You still need to kill and relaunch OmniCode to load the new build.
...
This works on **all** VS Code builds (OmniCode, Insiders, stable):
...
However, **`fill` silently fails on OmniCode** — the command completes without error but no text appears. Always verify text appeared after typing, and fall back to the keyboard shortcut + `press` pattern if it didn't. The `press`-per-key approach works universally across all builds.

### Fastest Method on OmniCode / Agents Window: Clipboard Paste (macOS only)
...
| Method | VS Code Insiders | OmniCode |
|--------|-----------------|----------|
...
- Make sure OmniCode was launched with `--remote-debugging-port=NNNN`
- If OmniCode was already running, quit and relaunch with the flag
...
- `fill` and `type` work on VS Code Insiders but **silently fail on OmniCode** — they complete without error but no text appears. The `press`-per-key approach works universally.
- See the "Interacting with Monaco Editor" section above for the full compatibility matrix.

## Cleanup

**Always kill the OmniCode instance when you're done.** OmniCode is a full Electron app that consumes significant memory (often 1–4 GB+). Leaving it running wastes resources and holds the CDP port.

```bash
# Disconnect npx @playwright/cli
npx @playwright/cli close

# Kill the OmniCode instance listening on the debug port (if running)
# macOS / Linux:
pids=$(lsof -t -i :9224)
if [ -n "$pids" ]; then
	kill $pids
...
# Windows:
# taskkill /F /PID <PID>
# Or use Task Manager to end "OmniCode"
```
