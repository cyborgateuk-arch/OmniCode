...
export async function GET(request: Request) {
  ...
  try {
    ...
    const loopbackBypass = isLoopbackRequest(request);
    const requireLogin = loopbackBypass ? false : settings.requireLogin !== false;
    const hasPassword = loopbackBypass ? false : hasManagementPasswordConfigured(settings);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const allowLoopbackDisable = requireLogin === false && isLoopbackRequest(request);

    if (
      !isBootstrapSecurityWindow(settings) &&
      !allowLoopbackDisable &&
    ...
    if (typeof requireLogin === "boolean") {
      ...
      if (requireLogin === false && allowLoopbackDisable) {
        updates.password = "";
      }
    }

    if (password && requireLogin !== false) {
