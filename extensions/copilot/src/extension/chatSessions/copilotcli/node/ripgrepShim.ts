...
async function _ensureRipgrepShim(extensionPath: string, vscodeAppRoot: string, logService: ILogService): Promise<void> {
	const vscodeRipgrepPath = await resolveVSCodeRipgrepPath(vscodeAppRoot, logService);

	await copyRipgrepShim(extensionPath, vscodeRipgrepPath, logService);
}

async function resolveVSCodeRipgrepPath(vscodeAppRoot: string, logService: ILogService): Promise<string> {
	try {
		const ripgrep = await import('@vscode/ripgrep');
		if (typeof ripgrep.rgPath === 'string' && ripgrep.rgPath.length > 0) {
			const resolvedDir = path.dirname(ripgrep.rgPath);
			logService.trace(`Resolved ripgrep shim source from @vscode/ripgrep: ${resolvedDir}`);
			return resolvedDir;
		}
	} catch (error) {
		logService.warn(`Failed to resolve @vscode/ripgrep rgPath directly: ${error instanceof Error ? error.message : String(error)}`);
	}

	const candidates = [
		path.join(vscodeAppRoot, 'node_modules', '@vscode', 'ripgrep', 'bin'),
		path.join(vscodeAppRoot, 'build', 'node_modules', '@vscode', 'ripgrep', 'bin'),
		path.join(vscodeAppRoot, 'remote', 'node_modules', '@vscode', 'ripgrep', 'bin'),
	];
	...
	for (const candidate of candidates) {
		try {
			...
			if (stat.isDirectory()) {
				logService.trace(`Resolved ripgrep shim source from fallback path: ${candidate}`);
				return candidate;
			}
		} catch {
	...
	throw new Error(`Could not resolve a VS Code ripgrep binary directory from ${vscodeAppRoot}`);
}
