...
describe('ChatSessionInitializer', () => {
	...
	describe('resolveModelId', () => {
		...
		it('passes through non-copilot request models when they are not in the copilot registry', async () => {
			const { initializer } = createInitializer();
			const request = makeRequest({ model: { id: 'gemini-cli/gemini-3.1-pro-preview', vendor: 'customendpoint' } } as Partial<vscode.ChatRequest>);
			const result = await initializer.resolveModel(request, CancellationToken.None);
			expect(result).toEqual(expect.objectContaining({ model: 'gemini-cli/gemini-3.1-pro-preview' }));
		});
