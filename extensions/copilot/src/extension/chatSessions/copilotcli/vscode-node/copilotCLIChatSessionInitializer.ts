...
export class CopilotCLIChatSessionInitializer implements ICopilotCLIChatSessionInitializer {
	...
	async resolveModel(request: vscode.ChatRequest | undefined, token: vscode.CancellationToken): Promise<{ model: string; reasoningEffort?: string } | undefined> {
		...
		if (request?.model?.id && request.model.vendor !== 'copilot' && request.model.vendor !== 'copilotcli') {
			return {
	...
	private async getModelFromPromptFile(models: readonly string[]): Promise<string | undefined> {
		for (const model of models) {
			...
			modelId = await this.resolveExtensionContributedModel(model);
			if (modelId) {
				return modelId;
			}
			...
			const strippedModel = model.substring(0, model.indexOf('(')).trim();
			modelId = await this.copilotCLIModels.resolveModel(strippedModel);
			if (modelId) {
				return modelId;
			}
			modelId = await this.resolveExtensionContributedModel(strippedModel);
			if (modelId) {
				return modelId;
			}
		}
	...
	private async resolveExtensionContributedModel(modelName: string): Promise<string | undefined> {
		const availableModels = await vscode.lm.selectChatModels();
		const normalized = modelName.trim().toLowerCase();
		const match = availableModels.find(model => {
			const qualifiedName = `${model.name} (${model.vendor})`.trim().toLowerCase();
			return model.id.trim().toLowerCase() === normalized
				|| model.name.trim().toLowerCase() === normalized
				|| qualifiedName === normalized;
		});
		if (!match || match.vendor === 'copilot' || match.vendor === 'copilotcli') {
			return undefined;
		}
		return match.id;
	}
