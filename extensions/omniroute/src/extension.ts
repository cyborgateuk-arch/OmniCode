...
const OMNIROUTE_EMBEDDED_RUNTIME_DIR = 'omniproxy-runtime';
...
class OmniRouteService implements vscode.Disposable, vscode.TreeDataProvider<TreeNode> {
	...
	private resolveOmniRouteRoot(): string {
		...
		return path.join(this.repoRoot, OMNIROUTE_EMBEDDED_RUNTIME_DIR);
	}
	...
	private getEmbeddedNodePath(): string | undefined {
		...
		const appBinary = path.join(this.repoRoot, '.build', 'electron', 'OmniCode.app', 'Contents', 'MacOS', 'OmniCode');
	...
	private async ensureServer(options: { readonly silent: boolean }): Promise<boolean> {
		...
		await this.ensureRuntimeOAuthDefaults();
	...
	private async ensureRuntimeOAuthDefaults(): Promise<void> {
		const exitCode = await this.runNodeCommand(['scripts/sync-env.mjs', '--oauth-only'], { logOutput: false });
		if (exitCode !== 0) {
			this.outputChannel.appendLine(`[${OMNIPROXY_BRAND_NAME}] failed to sync OAuth defaults into runtime .env`);
		}
	}
	...
	private async recoverStalePortListener(): Promise<void> {
		...
		const cwd = info.cwd ?? '';
		const looksLikeOmniRuntime = /run-next\.mjs|npm run dev/.test(command) || /OmniRoute-main|omniproxy-runtime/.test(cwd);
