# OmniCode Development MCP Server

This directory contains a Model Context Protocol (MCP) server that provides VS Code automation capabilities for OmniCode development and testing. The MCP server exposes OmniCode's testing infrastructure through a standardized interface, allowing AI assistants and other tools to interact with VS Code programmatically.
...
### Server Won't Start
- Ensure OmniCode has been built and run at least once (via F5 or `code.sh`)
...
This MCP server is part of the OmniCode development infrastructure. When making changes:
