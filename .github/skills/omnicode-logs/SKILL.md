---
name: omnicode-logs
description: 'Find and read logs from OmniCode dev builds. Use when: finding logs, reading log files, debugging OmniCode, checking renderer logs, extension host logs, agent host logs, main process logs, investigating errors in dev builds.'
---

# OmniCode Logs

Find and display logs from the most recent OmniCode or Agents app dev run.

## Log Root Directories

| App | Default User Data Dir | Logs Path |
|-----|-----------------------|-----------|
| OmniCode | `$HOME/.vsomnicode` | `$HOME/.vsomnicode/logs/` |
| Agents app | `$HOME/.vsomnicode` | `$HOME/.vsomnicode/logs/` |

If OmniCode was launched with `--user-data-dir=<dir>`, use `<dir>/logs/` instead of the defaults above. Launch and debugging helpers often create temporary user data dirs under `.build/`; always prefer the exact user data dir from the launch command when it is known.
...
1. **Identify which app** the user is asking about: OmniCode or Agents app. If unclear, check both.
2. **Find the most recent log folder**:
    ```bash
    ls -lt "$HOME/.vsomnicode/logs" | head -5
...
When using temporary `console.log` probes and you need those probes to persist in the normal log files, enable dev console forwarding locally before launching OmniCode.
...
4. Launch OmniCode or the Agents app and reproduce the issue.
