...
export class ExtensionEnablementService extends Disposable implements IWorkbenchExtensionEnablementService {
	...
	private ensureChatExtensionInitialDisabledState(): void {
		if (!this._chatExtensionId || this._isSameUnifiedExtension() || this.environmentService.isSessionsWindow || this.environmentService.skipBuiltinExtensions?.some(id => id.toLowerCase() === this._chatExtensionId)) {
			return;
		}
	...
	private _isDisabledByUnification(identifier: IExtensionIdentifier): boolean {
		return this._extensionUnificationEnabled && !this._isSameUnifiedExtension() && identifier.id.toLowerCase() === this._completionsExtensionId;
	}

	private _isSameUnifiedExtension(): boolean {
		return !!this._completionsExtensionId && this._completionsExtensionId === this._chatExtensionId;
	}
