 ...
 *
 * Steps:
 * 1. Sign In — sessions-style sign-in hero with OmniCode, Google, and Apple options
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to OmniCode"));
		...
		// Add OmniCode logo to header
		const brandIcon = append(header, $('span.onboarding-a-header-brand-icon'));
	...
	private _updateButtonStates(): void {
		...
		if (this.nextButton) {
			if (this.currentStepIndex === 0) {
				// Sign-in step: secondary "Continue"
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to OmniCode");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "Get started with AI-powered development.");

		// Sign-in buttons and disclaimer removed
	}
...
*/
	...
	private _renderAgentSessionsStep(container: HTMLElement): void {
		...
		this._createFeatureCard(moreGrid, Codicon.rocket,
			...
			localize('onboarding.sessions.runAnywhere.desc', "Run agents locally for interactive work, in the background with OmniCode Agent CLI, or in the cloud with cloud agents that open a pull request your team can review."));

		this._createFeatureCard(moreGrid, Codicon.settingsGear,
			localize('onboarding.sessions.customize', "Customize Your Agents"),
			localize('onboarding.sessions.customize.desc', "Tailor OmniCode AI to your project with custom instructions and agents, skills, reusable prompts, and MCP servers that connect to the tools and context you rely on."));
