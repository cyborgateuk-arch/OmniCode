...
/**
 * @deprecated Legacy vendor id for the GitHub Copilot language model provider.
 * Kept for backward compatibility with existing extensions and storage keys.
 * OmniCode treats all vendors equally — no single vendor is hardcoded as default.
 */
...
export namespace ILanguageModelChatMetadata {
	...
	export function matchesQualifiedName(name: string, metadata: ILanguageModelChatMetadata): boolean {
		// Match by unqualified name for any vendor, or by fully qualified name
 ...
 */
const AUTO_MODEL_IDENTIFIER = 'auto';
...
export class LanguageModelsService implements ILanguageModelsService {
	...
	constructor(
		...
		this._store.add(this.onDidChangeLanguageModels(() => {
			...
			let hasNonDefaultUserSelectable = false;
			const defaultVendor = this._vendors.values().next().value;
			for (const model of this._modelCache.values()) {
				if (model.isUserSelectable === false) {
					continue;
				}
				hasUserSelectable = true;
				if (!defaultVendor || model.vendor !== defaultVendor.vendor) {
					hasNonDefaultUserSelectable = true;
					break;
				}
			}
			this._hasUserSelectableModels.set(hasUserSelectable);
			this._hasNonCopilotUserSelectableModels.set(hasNonDefaultUserSelectable);
	...
	deltaLanguageModelChatProviderDescriptors(added: IUserFriendlyLanguageModel[], removed: IUserFriendlyLanguageModel[]): void {
		...
		for (const item of added) {
			...
			const vendor: ILanguageModelProviderDescriptor = {
				...
				when: item.when,
				isDefault: this._vendors.size === 0
			};
	...
	async configureLanguageModelsProviderGroup(vendorId: string, providerGroupName?: string): Promise<void> {
		...
		try {
			const configuration = vendor.configuration ? await this.promptForConfiguration(name, vendorId, vendor.configuration, existingConfiguration) : undefined;
			...
			if (vendor.configuration && vendorId !== 'customendpoint' && this.requireConfiguring(vendor.configuration)) {
	...
	private async promptForConfiguration(groupName: string, vendorId: string, configuration: IJSONSchema, existing: IStringDictionary<unknown> | undefined): Promise<IStringDictionary<unknown> | undefined> {
		if (!configuration.properties) {
			return;
		}

		if (vendorId === 'customendpoint') {
			return this.promptForCustomEndpointConfiguration(groupName, configuration, existing);
		}
	...
	private async promptForCustomEndpointConfiguration(groupName: string, configuration: IJSONSchema, existing: IStringDictionary<unknown> | undefined): Promise<IStringDictionary<unknown> | undefined> {
		const apiKeySchema = configuration.properties?.apiKey;
		const apiKeyRequired = !!configuration.required?.includes('apiKey');
		const apiKey = await this.promptForValue(groupName, 'apiKey', apiKeySchema, apiKeyRequired, existing);
		if (apiKey === undefined || !isString(apiKey)) {
			return undefined;
		}

		const existingModel = Array.isArray(existing?.models) && isObject(existing.models[0]) ? existing.models[0] : undefined;
		const existingUrl = isString(existingModel?.url) ? existingModel.url : undefined;
		const urlSchema: IJSONSchema = {
			type: 'string',
			title: 'URL',
			pattern: '^https?://.+',
			patternErrorMessage: 'URL must start with http:// or https://',
			description: 'Base URL for the custom endpoint'
		};
		const url = await this.promptForInput(groupName, 'url', urlSchema, true, { url: existingUrl });
		if (url === undefined || !isString(url)) {
			return undefined;
		}

		const resolvedApiKey = await this.resolveSecretValue(apiKey);
		if (!isString(resolvedApiKey) || isFalsyOrWhitespace(resolvedApiKey)) {
			throw new Error(localize('customEndpointModelsMissingApiKey', "The API key secret reference could not be resolved."));
		}

		let currentUrl = url;
		while (true) {
			const selectedModel = await this.promptForCustomEndpointModel(groupName, currentUrl, resolvedApiKey, isString(existingModel?.id) ? existingModel.id : undefined);
			if (!selectedModel) {
				return undefined;
			}
			if (selectedModel.kind === 'retry') {
				continue;
			}
			if (selectedModel.kind === 'editUrl') {
				const updatedUrl = await this.promptForInput(groupName, 'url', urlSchema, true, { url: currentUrl });
				if (updatedUrl === undefined || !isString(updatedUrl)) {
					return undefined;
				}
				currentUrl = updatedUrl;
				continue;
			}
			if (selectedModel.kind === 'manual') {
				const manualModel = await this.promptForCustomEndpointManualModel(groupName, existingModel);
				if (!manualModel) {
					return undefined;
				}
				return {
					apiKey,
					models: [this.createCustomEndpointModelConfiguration(manualModel, currentUrl, existingModel)]
				};
			}

			return {
				apiKey,
				models: [this.createCustomEndpointModelConfiguration(selectedModel.model, currentUrl, existingModel)]
			};
		}
	}
	...
	private async promptForValue(groupName: string, property: string, propertySchema: IJSONSchema | undefined, required: boolean, existing: IStringDictionary<unknown> | undefined): Promise<unknown | undefined> {
		...
		if (this.canPromptForObjectArrayProperty(propertySchema)) {
			return this.promptForObjectArray(groupName, property, propertySchema, existing);
		}
	...
	private canPromptForObjectArrayProperty(propertySchema: IJSONSchema | undefined): boolean {
		if (!propertySchema || typeof propertySchema === 'boolean') {
			return false;
		}
		if (propertySchema.type !== 'array' || !propertySchema.items || Array.isArray(propertySchema.items) || typeof propertySchema.items === 'boolean') {
			return false;
		}
		...
		return requiredKeys.every(key => this.canPromptForProperty(itemSchema.properties?.[key]))
			&& propertyKeys.some(key => this.canPromptForProperty(itemSchema.properties?.[key]));
	}

	private async promptForObjectArray(groupName: string, property: string, propertySchema: IJSONSchema, existing: IStringDictionary<unknown> | undefined): Promise<IStringDictionary<unknown>[] | undefined> {
		if (!propertySchema.items || Array.isArray(propertySchema.items) || typeof propertySchema.items === 'boolean') {
			return undefined;
		}
		...
		const existingItems = Array.isArray(existing?.[property]) ? existing[property] : undefined;
		const firstExisting = existingItems && typeof existingItems[0] === 'object' && existingItems[0] !== null
			? existingItems[0] as IStringDictionary<unknown>
			: undefined;
		const result: IStringDictionary<unknown> = firstExisting ? { ...firstExisting } : {};
		const promptGroupName = `${groupName}: ${propertySchema.title ?? property}`;

		for (const itemProperty of Object.keys(itemSchema.properties)) {
			const itemPropertySchema = itemSchema.properties[itemProperty];
			if (!this.canPromptForProperty(itemPropertySchema)) {
				continue;
			}

			const itemRequired = !!itemSchema.required?.includes(itemProperty);
			const value = await this.promptForValue(promptGroupName, itemProperty, itemPropertySchema, itemRequired, firstExisting);
			if (value !== undefined) {
				result[itemProperty] = value;
			}
		}

		const requiredKeys = itemSchema.required ?? [];
		if (!requiredKeys.every(key => result[key] !== undefined)) {
			return undefined;
		}

		const remainingItems = existingItems?.slice(1) ?? [];
	...
	private async fetchCustomEndpointModels(url: string, apiKey: string): Promise<Array<{ id: string; name: string; toolCalling?: boolean; vision?: boolean; maxInputTokens?: number; maxOutputTokens?: number }>> {
		const context = await this._requestService.request({
			type: 'GET',
			url: this.getCustomEndpointModelsUrl(url),
			headers: {
				Accept: 'application/json',
				...(apiKey ? { Authorization: `Bearer ${apiKey}` } : {})
			},
			callSite: 'languageModels.fetchCustomEndpointModels'
		}, CancellationToken.None);

		if (context.res.statusCode !== 200) {
			throw new Error(localize('customEndpointModelsFetchFailedStatus', "Failed to fetch models from the custom endpoint ({0}).", context.res.statusCode ?? 'unknown'));
		}

		const result = await asJson<unknown>(context);
		const resultObject = isObject(result) ? result as IStringDictionary<unknown> : undefined;
		const entries: unknown[] | undefined = Array.isArray(result)
			? result
			: resultObject && Array.isArray(resultObject.data)
				? resultObject.data
				: undefined;

		if (!entries) {
			throw new Error(localize('customEndpointModelsFetchFailedShape', "The custom endpoint did not return a valid models list."));
		}

		const models = entries
			.filter((entry): entry is IStringDictionary<unknown> => isObject(entry) && isString((entry as IStringDictionary<unknown>).id))
			.map(entry => {
				const capabilities = isObject(entry.capabilities) ? entry.capabilities as IStringDictionary<unknown> : undefined;
				return {
					id: String(entry.id),
					name: isString(entry.display_name) ? entry.display_name : isString(entry.name) ? entry.name : String(entry.id),
					toolCalling: typeof entry.toolCalling === 'boolean'
						? entry.toolCalling
						: typeof capabilities?.tool_calling === 'boolean'
							? capabilities.tool_calling
							: undefined,
					vision: typeof entry.vision === 'boolean'
						? entry.vision
						: typeof capabilities?.vision === 'boolean'
							? capabilities.vision
							: undefined,
					maxInputTokens: typeof entry.maxInputTokens === 'number'
						...
						: typeof entry.max_input_tokens === 'number'
							...
							: typeof entry.context_window === 'number'
								...
								: typeof entry.context_length === 'number'
									...
									: undefined,
					maxOutputTokens: typeof entry.maxOutputTokens === 'number'
		...
		if (!models.length) {
			throw new Error(localize('customEndpointModelsEmpty', "No models were returned by the custom endpoint."));
		}

		return models;
	}

	private getCustomEndpointModelsUrl(url: string): string {
		const endpoint = new URL(url);
		const pathname = endpoint.pathname.replace(/\/$/, '');
		const explicitPath = /(\/chat\/completions|\/responses|\/messages)$/;
		endpoint.pathname = explicitPath.test(pathname)
			? pathname.replace(explicitPath, '/models')
			: `${pathname}/models`;
		return endpoint.toString();
	}

	private async promptForCustomEndpointModel(
		groupName: string,
		url: string,
		apiKey: string,
		existingModelId?: string
	): Promise<
		| { kind: 'selected'; model: { id: string; name: string; toolCalling?: boolean; vision?: boolean; maxInputTokens?: number; maxOutputTokens?: number } }
		| { kind: 'retry' }
		| { kind: 'editUrl' }
		| { kind: 'manual' }
		| undefined
	> {
		type ModelItem = IQuickPickItem & {
			kind: 'model';
			model: { id: string; name: string; toolCalling?: boolean; vision?: boolean; maxInputTokens?: number; maxOutputTokens?: number };
		};
		type ActionItem = IQuickPickItem & {
			kind: 'action';
			action: 'retry' | 'editUrl' | 'manual' | 'cancel';
		};
		type QuickPickItem = ModelItem | ActionItem;
		const disposables = new DisposableStore();
		try {
			return await new Promise(resolve => {
				const quickPick = disposables.add(this._quickInputService.createQuickPick<QuickPickItem>());
				let settled = false;
				const finish = (
					result:
						| { kind: 'selected'; model: { id: string; name: string; toolCalling?: boolean; vision?: boolean; maxInputTokens?: number; maxOutputTokens?: number } }
				...
				quickPick.title = `${groupName}: Model`;
				quickPick.items = [];
				quickPick.busy = true;
				quickPick.placeholder = localize('customEndpointFetchInProgress', "Fetching models from the custom endpoint…");
				quickPick.ignoreFocusOut = true;
				...
				disposables.add(quickPick.onDidAccept(() => {
					...
					if (selected.kind === 'action') {
						if (selected.action === 'retry') {
							finish({ kind: 'retry' });
						} else if (selected.action === 'editUrl') {
							finish({ kind: 'editUrl' });
						...
						finish({ kind: 'selected', model: selected.model });
					}
					quickPick.hide();
				}));
				disposables.add(quickPick.onDidHide(() => finish(undefined)));
				quickPick.show();

				void (async () => {
					try {
						const models = await this.fetchCustomEndpointModels(url, apiKey);
						if (settled) {
							return;
						}
						...
						const activeItem = existingModelId ? items.find(item => item.model.id === existingModelId) : undefined;

						quickPick.busy = false;
						quickPick.items = items;
						quickPick.placeholder = localize('customEndpointSelectModel', "Select which model to use");
						if (activeItem) {
						...
						quickPick.items = [
							{
								kind: 'action',
								label: localize('customEndpointRetryFetch', "Retry fetching models"),
								description: message,
								action: 'retry'
							},
							{
								kind: 'action',
								label: localize('customEndpointEditUrl', "Edit URL"),
								description: localize('customEndpointEditUrlDescription', "Change the endpoint URL and try again"),
								action: 'editUrl'
							},
							{
								kind: 'action',
								label: localize('customEndpointAddModelManually', "Add model manually"),
								description: localize('customEndpointAddModelManuallyDescription', "Save a model id without fetching the endpoint"),
								action: 'manual'
							},
							{
								kind: 'action',
								label: localize('customEndpointCancel', "Cancel"),
								action: 'cancel'
							}
						];
						quickPick.placeholder = localize('customEndpointFetchFailed', "Could not fetch models. Select an action to continue.");
					}
				})();
			});
		} finally {
	...
	private createCustomEndpointModelConfiguration(
		model: { id: string; name: string; toolCalling?: boolean; vision?: boolean; maxInputTokens?: number; maxOutputTokens?: number },
		url: string,
		existingModel: IStringDictionary<unknown> | undefined
	): IStringDictionary<unknown> {
		return {
			id: model.id,
			name: model.name,
			url,
			toolCalling: typeof existingModel?.toolCalling === 'boolean' ? existingModel.toolCalling : model.toolCalling ?? true,
			vision: typeof existingModel?.vision === 'boolean' ? existingModel.vision : model.vision ?? false,
			maxInputTokens: typeof existingModel?.maxInputTokens === 'number' ? existingModel.maxInputTokens : model.maxInputTokens ?? 128000,
			maxOutputTokens: typeof existingModel?.maxOutputTokens === 'number' ? existingModel.maxOutputTokens : model.maxOutputTokens ?? 16000
		};
	}

	private async promptForCustomEndpointManualModel(
		groupName: string,
		existingModel: IStringDictionary<unknown> | undefined
	): Promise<{ id: string; name: string } | undefined> {
		const modelIdSchema: IJSONSchema = {
			type: 'string',
			title: localize('customEndpointManualModelIdTitle', "Model ID"),
			description: localize('customEndpointManualModelIdDescription', "Enter the model identifier to save manually.")
		};
		const modelId = await this.promptForInput(`${groupName}: Model`, 'id', modelIdSchema, true, { id: existingModel?.id });
		if (modelId === undefined || !isString(modelId)) {
			return undefined;
		}

		const modelNameSchema: IJSONSchema = {
			type: 'string',
			title: localize('customEndpointManualModelNameTitle', "Model Name"),
			description: localize('customEndpointManualModelNameDescription', "Optional display name. Leave empty to use the model id.")
		};
		const modelName = await this.promptForInput(`${groupName}: Model`, 'name', modelNameSchema, false, { name: existingModel?.name });
		return {
			id: modelId,
			name: isString(modelName) && !isFalsyOrWhitespace(modelName) ? modelName : modelId
		};
	}
	...
	private decodeSecretKey(secretInput: unknown): string | undefined {
		...
		const match = /^\$\{input:(chat\.lm\.secret\.[^}]+)\}$/.exec(secretInput);
		return match?.[1];
	}

	private async resolveSecretValue(value: unknown): Promise<unknown> {
		if (!isString(value)) {
			return value;
		}
		...
		return this._secretStorageService.get(secretKey);
	}
	...
	private async _resolveConfiguration(group: ILanguageModelsProviderGroup, schema: IJSONSchema | undefined): Promise<IStringDictionary<unknown>> {
		...
		for (const key in group) {
			...
			if (schema.properties?.[key]?.secret) {
				value = await this.resolveSecretValue(value);
			}
			result[key] = value;
		}
