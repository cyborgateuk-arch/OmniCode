...
suite('HasByokModelsContribution', () => {
	...
	test('configured providers enable BYOK even when clientByokEnabled=false', async () => {
	...
	test('configured providers enable BYOK even when offlineByok=false', async () => {
		const store = disposables.add(new DisposableStore());
		const scenario = createScenario(store, {
			groups: [{ vendor: 'ollama', name: 'Ollama' }],
	...
	test('any configured provider group counts as BYOK (vendor-neutral)', async () => {
		const store = disposables.add(new DisposableStore());
		const scenario = createScenario(store, {
		...
		// All vendors are treated equally — any configured group enables BYOK
		assert.deepStrictEqual(snapshot(scenario, true), { hasByokModels: true, persistedLastKnown: true });
	});
