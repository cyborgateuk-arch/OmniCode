...
suite('DefaultModelContribution', () => {
	...
	test('customendpoint and customoai vendors are included even if not in visibleVendors', () => {
		const { arrays } = setup({
			storageFormat: 'vendorAndId',
			vendors: [{ vendor: 'copilot', displayName: 'Copilot', isDefault: true, configuration: undefined, managementCommand: undefined, when: undefined }],
			models: [
				makeMetadata({ id: 'gpt-4o-mini', name: 'GPT 4o mini', vendor: 'copilot' }),
				makeMetadata({ id: 'claude-sonnet', name: 'Claude Sonnet', vendor: 'customendpoint' }),
				makeMetadata({ id: 'gpt-4o-custom', name: 'GPT 4o Custom', vendor: 'customoai' }),
				makeMetadata({ id: 'hidden-model', name: 'Hidden Model', vendor: 'hidden-vendor' }),
			],
		});
		assert.deepStrictEqual(
			{ ids: arrays.modelIds, labels: arrays.modelLabels },
			{
				ids: ['', 'customendpoint/claude-sonnet', 'customoai/gpt-4o-custom', 'copilot/gpt-4o-mini'],
				labels: ['Auto (Vendor Default)', 'Claude Sonnet (Custom Endpoint)', 'GPT 4o Custom (Custom OpenAI)', 'GPT 4o mini (Copilot)'],
			},
		);
	});
});
