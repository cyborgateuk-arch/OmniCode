...
export class OpenWorkspaceInAgentsWindowAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('openWorkspaceInAgentsWindow', "OmniProxy"),
	...
	async run(accessor: ServicesAccessor) {
		...
		await commandService.executeCommand('workbench.action.omniProxy.manage');
	}
}
...
export class OpenAgentsWindowAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('openAgentsWindow', "Open OmniCode Agents Window"),
			category: CHAT_CATEGORY,
			precondition: OPEN_AGENTS_WINDOW_PRECONDITION,
			f1: true,
			keybinding: [{
...
/**
 * Renders the "OmniProxy" titlebar entry as an icon-only button that
...
class OpenWorkspaceInAgentsTitleBarWidget extends BaseActionViewItem {
	...
	override render(container: HTMLElement): void {
		...
		const hoverText = localize('openInAgentsHover', "Open OmniProxy");
