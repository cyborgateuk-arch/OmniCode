...
// Vendor-neutral — all non-default vendors are treated as BYOK
 ...
 *  - the `LanguageModelsService` has at least one resolved, user-selectable non-default model
...
export class HasByokModelsContribution extends Disposable implements IWorkbenchContribution {
	...
	private _isFeatureEnabled(): boolean {
		...
		return this._hasConfiguredByokProvider()
			|| (
	...
	private _hasConfiguredByokProvider(): boolean {
		return this._languageModelsConfigurationService.getLanguageModelsProviderGroups().length > 0;
	}
	...
	private _restore(): void {
		...
		if (this._hasConfiguredByokProvider()) {
	...
	private _update(): void {
		...
		const hasByokVendor = this._languageModelsConfigurationService.getLanguageModelsProviderGroups().length > 0;
		if (!hasByokVendor) {
			this._setResult(false);
		}
	}
