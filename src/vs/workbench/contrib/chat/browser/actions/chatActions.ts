...
const defaultChat = {
	provider: Object.assign({ enterprise: { id: '' } }, product.defaultChatAgent?.provider),
	...
	registerAction2(class ShowQuotaExceededDialogAction extends Action2 {
		...
		constructor() {
			super({
				...
				title: localize('upgradeChat', "Manage AI Models")
			});
		}
		...
		override async run(accessor: ServicesAccessor) {
			...
			const manageModelsMessage = localize('manageModelsMessage', "Open model management to configure custom providers, local models, and available premium models.");

			await dialogService.prompt({
				type: 'none',
				message: localize('copilotQuotaReached', "AI model quota reached"),
				cancelButton: {
				...
				custom: {
					...
					markdownDetails: coalesce([
						...
						{ markdown: new MarkdownString(manageModelsMessage, true) }
					])
				}
			});
		}
