...
import { filterModelsForSession, findDefaultModel } from '../widget/input/chatModelSelectionLogic.js';
...
export class ConfigureAgentModelAction extends Action2 {
	static readonly ID = 'workbench.action.chat.configureAgentModel';

	constructor() {
		super({
			id: ConfigureAgentModelAction.ID,
			title: localize2('interactive.configureAgentModel.label', "Configure Agent Model"),
			tooltip: localize('configureAgentModelTooltip', "Configure custom model for Agent"),
			category: CHAT_CATEGORY,
			f1: false,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const allModelIds = languageModelsService.getLanguageModelIds();
		...
		const sessionType = widget.viewModel?.model.sessionResource ? getChatSessionType(widget.viewModel.model.sessionResource) : localChatSessionType;
		const supportedModels = filterModelsForSession(models, sessionType, ChatModeKind.Agent, ChatAgentLocation.Chat);
		...
		const currentModelId = widget.input.currentLanguageModel ?? '';
		const defaultModel = findDefaultModel(supportedModels, ChatAgentLocation.Chat);

		// Prepare QuickPick items
		const items: QuickPickInput<IQuickPickItem & { modelId?: string; isSettingsOpener?: boolean }>[] = [];

		// Option 1: Auto (current default model)
		items.push({
			id: '',
			modelId: '',
			label: localize('defaultModel', 'Auto'),
			description: defaultModel ? localize('defaultModelDescription.withValue', "Use the current default model: {0}", defaultModel.metadata.name) : localize('defaultModelDescription', "Use the current default model"),
			alwaysShow: true,
		});

		for (const model of supportedModels) {
			const vendorDisplayName = vendorDisplayNames.get(model.metadata.vendor) ?? (
				model.metadata.vendor === 'customendpoint' ? localize('customEndpointVendor', "Custom Endpoint") :
				model.metadata.vendor === 'customoai' ? localize('customOAIVendor', "Custom OpenAI") :
			...
			items.push({
				...
				detail: model.metadata.tooltip ?? model.metadata.detail ?? '',
			});
		}
		...
		items.push({
			id: 'configure_in_settings',
			isSettingsOpener: true,
			label: localize('manageModels', 'Manage Models...'),
			description: localize('manageModelsDescription', "Open language model management"),
			alwaysShow: true,
		});

		const quickPick = quickInputService.createQuickPick<IQuickPickItem & { modelId?: string; isSettingsOpener?: boolean }>({ useSeparators: true });
		quickPick.title = localize('selectAgentModel', "Select Model for Agent");
		quickPick.placeholder = localize('selectAgentModelPlaceholder', "Search or select a model for the current agent chat");
		quickPick.items = items;
		quickPick.ignoreFocusOut = false;

		const activeItem = items.find((item): item is IQuickPickItem & { modelId?: string; isSettingsOpener?: boolean } => !!item && 'modelId' in item && item.modelId === currentModelId);
		if (activeItem) {
		...
		disposables.add(quickPick.onDidAccept(async () => {
			...
			quickPick.hide();
			if (selected) {
				if (selected.isSettingsOpener) {
					await commandService.executeCommand(MANAGE_CHAT_COMMAND_ID);
				} else if (typeof selected.modelId === 'string') {
					if (selected.modelId === '') {
						if (defaultModel) {
							widget.input.setCurrentLanguageModel(defaultModel);
						}
					} else {
						const metadata = languageModelsService.lookupLanguageModel(selected.modelId);
						if (metadata) {
							widget.input.setCurrentLanguageModel({ identifier: selected.modelId, metadata });
						}
					}
				}
			}
...
export function registerChatExecuteActions(): DisposableStore {
	...
	store.add(registerAction2(ConfigureAgentModelAction));
