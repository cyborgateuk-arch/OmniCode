...
/**
 * Shared implementation of "Open OmniCode Agent CLI State File". Resolves the
 ...
 * this helper after resolving the active agent CLI session resource.
 */
export async function openCopilotCliStateFile(
	...
	switch (result.kind) {
		case 'ok':
			...
			return;
		case 'no-session':
			notificationService.info(localize('openSessionEventsFile.noSession', "No OmniCode Agent CLI session is active."));
			return;
		case 'unsupported-scheme':
			notificationService.info(localize('openSessionEventsFile.unsupported', "The active chat session is not an OmniCode Agent CLI session."));
			return;
 ...
 * find the active agent CLI chat session. Suitable for vscode where the
...
export class OpenCopilotCliStateFileAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('openSessionEventsFile', "Open OmniCode Agent CLI State File"),
			f1: true,
