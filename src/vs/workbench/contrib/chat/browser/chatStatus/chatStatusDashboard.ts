...
export class ChatStatusDashboard extends DomWidget {
	...
	private renderSetupSection(): void {
		...
		const signedOut = this.chatEntitlementService.entitlement === ChatEntitlement.Unknown && !anonymousUser && !hasByokModels;
		...
		let descriptionText = localize('configureModelsDescription', "Add local or proxy-backed models to use AI features without signing in.");
		let descriptionClass = '.description';
		if (disabled) {
			descriptionText = localize('enableDescription', "Enable Copilot to use AI features.");
		}

		let buttonLabel = localize('manageModelsButton', "Manage Models");
		if (disabled) {
			buttonLabel = localize('enableCopilotButton', "Enable AI Features");
		}

		let commandId: string;
		if (newUser || anonymousUser || signedOut) {
