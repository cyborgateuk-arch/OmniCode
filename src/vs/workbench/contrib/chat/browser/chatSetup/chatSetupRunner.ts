...
const defaultChat = {
	...
	provider: Object.assign({ default: { id: '', name: '' }, enterprise: { id: '', name: '' }, apple: { id: '', name: '' }, google: { id: '', name: '' } }, product.defaultChatAgent?.provider),
...
export class ChatSetup {
	...
	private getButtons(options?: { forceSignInDialog?: boolean; forceAnonymous?: ChatSetupAnonymous }): Array<[string, ChatSetupStrategy, { styleButton?: (button: IButton) => void } | undefined]> {
		...
		if (!options?.forceAnonymous && !this.chatEntitlementService.anonymous && (this.context.state.entitlement === ChatEntitlement.Unknown || options?.forceSignInDialog)) {
	...
	private getDialogTitle(options?: { forceSignInDialog?: boolean; forceAnonymous?: ChatSetupAnonymous; dialogTitle?: string }): string {
		...
		if (this.chatEntitlementService.anonymous) {
			if (options?.forceAnonymous || !this.context.state.completed) {
	...
	private createDialogFooter(disposables: DisposableStore, options?: { forceAnonymous?: ChatSetupAnonymous }): HTMLElement {
		...
		if (options?.forceAnonymous || this.telemetryService.telemetryLevel === TelemetryLevel.NONE) {
			footer = localize('settingsAnonymous', "AI features are enabled without sign-in.");
		} else {
			footer = localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({0})"}'] }, "Manage custom providers, local models, and AI preferences from [settings]({0}) anytime.", this.defaultAccountService.resolveGitHubUrl(GitHubPaths.copilotSettings));
		}
