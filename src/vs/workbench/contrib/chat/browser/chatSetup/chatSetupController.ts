...
export class ChatSetupController extends Disposable {
	...
	private async doSetup(options: IChatSetupControllerOptions, watch: StopWatch): Promise<ChatSetupResultValue> {
		...
		try {
			const forceAnonymous = options.forceAnonymous
				?? (this.context.state.entitlement === ChatEntitlement.Unknown && this.configurationService.getValue<boolean>('chat.allowAnonymousAccess')
			...
			success = await this.install(entitlement ?? this.context.state.entitlement, watch, { ...options, forceAnonymous });
		} finally {
