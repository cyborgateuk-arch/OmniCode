...
export class GrowthSessionController extends Disposable implements IChatSessionItemController {
	...
	get items(): readonly IChatSessionItem[] {
		...
		return [{
			...
			label: localize('growthSession.label', "Explore AI Features"),
			description: localize('growthSession.description', "OmniCode AI is available with local models and custom providers."),
...
export class GrowthSessionOpenerParticipant implements ISessionOpenerParticipant {
	...
	async handleOpenSession(accessor: ServicesAccessor, session: IAgentSession, _openOptions?: ISessionOpenOptions): Promise<boolean> {
		...
		const opts: IChatViewOpenOptions = {
			...
			isPartialQuery: true,
			previousRequests: [{
				request: localize('growthSession.previousRequest', "What can OmniCode AI do?"),
				// allow-any-unicode-next-line
				response: localize('growthSession.previousResponse', "Welcome to OmniCode AI. Here are some things you can try:\n\n- \"Help me debug this error\"\n- \"Write tests for my function\"\n- \"Explain this code\"\n- \"Scaffold a REST API\"\n- \"Refactor this to be more readable\"\n\nYou can also connect custom providers and local models from the model management controls."),
			}],
		};
