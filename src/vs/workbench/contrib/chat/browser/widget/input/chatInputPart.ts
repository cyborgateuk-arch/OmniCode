...
import { IChatModelInputState, IChatRequestModeInfo, IInputModel, IChatModel } from '../../../common/model/chatModel.js';
...
import { ChatSessionPrimaryPickerAction, ChatSubmitAction, IChatExecuteActionContext, OpenDelegationPickerAction, OpenModelPickerAction, OpenModePickerAction, OpenPermissionPickerAction, OpenSessionTargetPickerAction, OpenWorkspacePickerAction, ConfigureAgentModelAction } from '../../actions/chatExecuteActions.js';
...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	private readonly _activeModel = observableValue<IChatModel | undefined>(this, undefined);
	...
	private getSelectedModelStorageKey(): string {
		...
		const modeSuffix = this.currentModeKind === ChatModeKind.Agent ? '.agent' : '';
		if (sessionType && this.hasModelsTargetingSessionType()) {
			return `chat.currentLanguageModel.${this.location}.${sessionType}${modeSuffix}`;
		}
		return `chat.currentLanguageModel.${this.location}${modeSuffix}`;
	}
	...
	private getSelectedModelIsDefaultStorageKey(): string {
		...
		const modeSuffix = this.currentModeKind === ChatModeKind.Agent ? '.agent' : '';
		if (sessionType && this.hasModelsTargetingSessionType()) {
			return `chat.currentLanguageModel.${this.location}.${sessionType}${modeSuffix}.isDefault`;
		}
		return `chat.currentLanguageModel.${this.location}${modeSuffix}.isDefault`;
	}

	private restoreSelectedModelForCurrentMode(): void {
	...
	private initSelectedModel() {
		this.restoreSelectedModelForCurrentMode();
	...
	private _createModelPickerDelegate(): IModelPickerDelegate {
		const activeComboModelObservable = derived(this, reader => {
			const model = this._activeModel.read(reader);
			if (!model) { return undefined; }
			return model.currentActiveComboModel.read(reader);
		});

		return {
			currentModel: this._currentLanguageModel,
			currentActiveComboModel: activeComboModelObservable,
	...
	*/
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		this._widget = widget;
		this._activeModel.set(widget.viewModel?.model, undefined);
		this.getVisibleOptionGroupsModeAndUpdateContextKeys(this.getCurrentSessionResource());
