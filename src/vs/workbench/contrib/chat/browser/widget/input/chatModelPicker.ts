	...
	if (useGroupedModelPicker) {
		...
		if (models.length) {
			...
			if (otherModels.length > 0) {
				...
				// Sort alphabetically by group name — no vendor preferencing
...
function createUnavailableModelItem(
	...
	if (reason === 'upgrade') {
		description = new MarkdownString(localize('chat.modelPicker.upgradeLink', "[Manage](command:workbench.action.chat.manage \" \")"), { isTrusted: true });
	...
	if (reason === 'upgrade') {
		...
		hoverContent.appendMarkdown(localize('chat.modelPicker.upgradeHover', "[Open model settings](command:workbench.action.chat.manage \" \") to manage available models and providers."));
	} else if (reason === 'update') {
