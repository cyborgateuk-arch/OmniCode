...
export abstract class InlineChatWidget {
	...
	#setupDisclaimer(): void {
		...
		this._store.add(autorun(reader => {
			...
			if (showDisclaimer) {
				const renderedMarkdown = disposables.add(this.#markdownRendererService.render(new MarkdownString(localize('termsDisclaimer', "Custom providers and local AI models are enabled without sign-in."), { isTrusted: true })));
