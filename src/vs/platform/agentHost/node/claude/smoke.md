...
- A real GitHub Copilot login. Models only populate after authenticate, and
  ...
  The `~/.vsomnicode-sessions-dev` user-data-dir caches login state across
...
```bash
LOG=$(ls -td ~/.vsomnicode-sessions-dev/logs/*/ | head -1)
...
The `~/.vsomnicode-sessions-dev` data dir is intentionally preserved so
